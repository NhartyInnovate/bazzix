...
def adjust_credits(db, user_id: int, amount: int, reference_id: str):
    if amount == 0:
    ...
    if amount < 0 and wallet.available_credits < abs(amount):
        raise InsufficientCreditsError("Insufficient credits to perform deduction")

    if amount > 0:
        wallet.purchased_balance += amount
    else:
        remaining_deduction = abs(amount)
        if wallet.subscription_balance >= remaining_deduction:
            wallet.subscription_balance -= remaining_deduction
        else:
            remaining_deduction -= wallet.subscription_balance
            wallet.subscription_balance = 0
            wallet.purchased_balance -= remaining_deduction
